...
test("generateCourseDraft fails when books are selected but not indexed", async () => {
  await assert.rejects(
    () => generateCourseDraft({
      titleHint: "Книга по управлению",
      audience: "Новые сотрудники",
      learningGoals: ["Опиши содержание книги"],
      durationMinutes: 45,
      rag: {
        enabled: true,
        documentIds: ["material_book_1"],
        topK: 6
      },
      generation: {
        provider: "template"
