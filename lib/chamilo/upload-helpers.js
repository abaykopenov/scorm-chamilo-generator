...
export function detectUploadOutcome({status, finalUrl, bodyText, history}) {
    ...
    // Empty body + status 200 on upload endpoint — Chamilo processed the file
    if (status >= 200 && status < 300 && /upload|newscorm|lp_controller/i.test(String(finalUrl || ""))) {
        return { ok: true, message: "Chamilo returned HTTP 200 on upload endpoint (empty response). Import likely succeeded." };
    }
...
export async function uploadScormWithForm({uploadForm, cookieJar, fileName, zipBuffer}) {
    ...
    // Debug: log what Chamilo returned
    console.log(`[chamilo-debug] Upload outcome:`, {
      ok: outcome.ok,
      message: outcome.message,
      status: finalStatus,
      finalUrl,
      bodySnippet: finalText.slice(0, 500),
      historyUrls: finalHistory.map(h => h?.url).filter(Boolean)
    });
...
export async function uploadScormToChamilo({zipBuffer, fileName, profile}) {
    ...
    // Always add the primary SCORM upload URL first
    const primaryScormUrl = buildUploadUrl(normalized);
    register(createSyntheticUploadForm(primaryScormUrl, normalized.courseCode, "user_file"));
    // Then add connection's found form (if different)
    register(connection.uploadForm);
    // Then add other fallbacks
