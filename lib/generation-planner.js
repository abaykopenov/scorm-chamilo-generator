...
function looksBrokenText(value) {
  ...
  // Pipe-separated TOC artifacts: "41 | | | 3.3.1 | Созданиеструктурысообщений ."
  if ((text.match(/\|/g) || []).length >= 2) {
    return true;
  }

  // Merged Cyrillic words (14+ consecutive Cyrillic without spaces)
  if (/[\u0400-\u04FF]{14,}/.test(text)) {
    return true;
  }
...
function looksLikeMetadataFact(text) {
  if (!text) return false;
  // Author credentials, publisher info, contact info
  if (/(?:Член[\-\s]*корреспондент|Почетный\s*работник|Почётный\s*работник|[Нн]аучный\s*редактор|[Дд]октор\s*(?:технических|физико|наук)|ISBN|©\s*\d{4}|[Сс]вязаться\s+с\s+\S+\s+можно)/i.test(text)) {
    return true;
  }
  // Check for garbled/merged Cyrillic words
...
function deriveObjectiveFromFacts(factTexts, fallbackLabel) {
  const texts = Array.isArray(factTexts) ? factTexts : [];
  // Find the first clean, non-metadata fact
  for (const text of texts) {
    const trimmed = `${text || ""}`.trim();
    if (trimmed.length >= 24 && !looksLikeMetadataFact(trimmed)) {
...
export function createPlannerScopedRagContext(plan, baseRagContext, scope) {
  ...
  const scopedHardMax = Number.isFinite(scopedHardMaxRaw) && scopedHardMaxRaw > 0
    ? Math.max(6, Math.min(24, Math.trunc(scopedHardMaxRaw)))
    : 16;
  const defaultTarget = hasSco ? 8 : (hasSection ? 10 : (hasModule ? 14 : 16));
