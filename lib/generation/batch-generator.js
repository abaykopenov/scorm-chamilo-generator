...
import { buildModuleMemory } from "./module-memory.js";
      ...
      try {
        ...
        for (let localScreenIndex = 0; localScreenIndex < partialScreens.length; localScreenIndex += 1) {
          ...
          accumulatedScreens.push({
            ...
            blocks: Array.isArray(original?.blocks) && original.blocks.length > 0
              ? original.blocks
              : [{ type: "text", text: `${original?.title || "Screen content"}` }]
          });
        }
...
export async function generateCourseByBatchesWithReset(input, ragContext, plannerPlan, hooks = {}) {
  ...
  for (let moduleIndex = modules.length; moduleIndex < size.moduleCount; moduleIndex += 1) {
    // Build shared memory from previously completed modules
    const moduleMemory = buildModuleMemory(modules);
...
export async function enrichRagContextForPlanner(input, ragContext, hooks) {
  ...
  const totalScreens = structure.totalScreens;
  // We want enough chunks to give each screen unique content
  // Target: at least 3 facts per screen slot, with a minimum of 60 and max of 200 chunks
  const minTargetChunks = Math.max(60, Math.min(200, totalScreens * 3));
  ...
  reportProgress(hooks, 7, "planner", `Enriching RAG: loading all material chunks for full coverage...`);

  try {
    // Import scroll function
    const { scrollAllMaterialChunks } = await import("../langchain-qdrant.js");
    
    const scrollResult = await scrollAllMaterialChunks({
      documentIds: input.rag.documentIds,
      embedding: input.rag.embedding
    });

    if (!scrollResult.ok || scrollResult.hits.length === 0) {
      // Fallback to old retry-based approach
      reportProgress(hooks, 8, "planner", `Scroll failed, falling back to search-based enrichment...`);
      return await enrichRagContextFallback(input, ragContext, hooks, minTargetChunks);
    }

    reportProgress(hooks, 9, "planner", `Loaded ${scrollResult.hits.length} total chunks from materials`);

    // Convert hits to chunk format
    const allChunks = scrollResult.hits
      .filter(hit => `${hit?.chunk?.text || ""}`.trim().length > 30)
      .map(hit => ({
    ...
    // Sample evenly across all chunks to cover the full books
    // If we have 1000 chunks and need 120, take every 8th chunk
    let sampled;
    if (allChunks.length <= minTargetChunks) {
      sampled = allChunks;
    } else {
      const step = allChunks.length / minTargetChunks;
      sampled = [];
      for (let i = 0; i < minTargetChunks; i++) {
        const index = Math.min(Math.floor(i * step), allChunks.length - 1);
        sampled.push(allChunks[index]);
      }
    }

    // Merge with semantic-search chunks (they remain important for relevance)
    const merged = mergeUniqueRagChunks([...currentChunks, ...sampled]);

    reportProgress(hooks, 10, "planner", `Enriched RAG: ${merged.length} chunks (from ${allChunks.length} total in materials)`);

    return {
      ...ragContext,
      chunks: merged,
      topK: Math.max(Number(ragContext?.topK) || 12, merged.length),
      message: `Full coverage: ${merged.length} chunks from ${allChunks.length} total material chunks`
    };
  } catch (error) {
    console.error("[enrichRagContextForPlanner] scroll error:", error?.message || error);
    return await enrichRagContextFallback(input, ragContext, hooks, minTargetChunks);
  }
}

// Fallback: the original retry-based enrichment (semantic search)
async function enrichRagContextFallback(input, ragContext, hooks, minTargetChunks) {
  const currentChunks = Array.isArray(ragContext?.chunks) ? ragContext.chunks : [];
  let best = ragContext;
  let merged = [...currentChunks];
  let topK = Math.max(Number(input?.rag?.topK) || 12, Number(ragContext?.topK) || 12);

  for (let retry = 1; retry <= 4; retry += 1) {
    topK = Math.min(40, topK + 6);
