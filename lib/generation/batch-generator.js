...
export async function enrichRagContextForPlanner(input, ragContext, hooks) {
  ...
  reportProgress(hooks, 7, "planner", `RAG context has ${currentChunks.length} chunks (target: ${minTargetChunks}). Using available chunks.`);
  
  // Note: Full material scrolling removed - using search-based retrieval only
  // For complete coverage, increase topK in RAG settings or use multiple queries
  return ragContext;
}
