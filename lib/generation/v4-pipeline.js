...
import { refineScreenText } from "./editor-agent.js";
import { 
  reportProgress, 
  buildEvidencePack, 
...
import { firstSentence, rotateList } from "../course-utils.js";
import { screenSlotId, getPlanSlotFacts } from "../generation-planner.js";
import { renderScreenFromFacts } from "./planner-builder.js";
...
async function writeAndCriticScreen({
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    reportProgress(hooks, 42, "writer", `Writer generates M${trace.module} S${trace.section} C${trace.sco} P${trace.screen} (attempt ${attempt}/${maxAttempts})`);

    // On retry, add critic feedback to help writer improve
    let writerInput = input;
    if (attempt > 1 && lastQuality?.summary) {
      writerInput = {
        ...input,
        previousBodyText: [
          input.previousBodyText || "",
          `[CRITIC FEEDBACK for retry ${attempt}]: ${lastQuality.summary}. FIX these issues in the next attempt.`
        ].filter(Boolean).join(" ")
      };
    }
    ...
    const candidateScore = computeCandidateScore(candidate, quality);
    if (candidateScore > bestScore) {
      bestScore = candidateScore;
      bestCandidate = candidate;
    }

    const traceLabel = `M${trace.module}.S${trace.section}.C${trace.sco}.P${trace.screen}`;

    if (quality.ok) {
      console.log(`[critic] ✅ APPROVED ${traceLabel} (attempt ${attempt}): ${(candidate.bodyLong || "").length} chars`);
      reportProgress(hooks, 44, "critic", `Critic approved ${traceLabel} (attempt ${attempt})`);
      return candidate;
    }

    const problemCodes = (quality.problems || []).map(p => `${p.code}(${p.severity})`).join(", ");
    console.log(`[critic] ❌ REJECTED ${traceLabel} (attempt ${attempt}/${maxAttempts}): ${problemCodes}`);
    reportProgress(hooks, 43, "critic", `Critic rejected ${traceLabel}: ${quality.reason} — retrying`);
  }

  // All attempts exhausted — use best candidate
  const traceLabel = `M${trace.module}.S${trace.section}.C${trace.sco}.P${trace.screen}`;
  console.warn(`[critic] ⚠️ FALLBACK ${traceLabel}: all ${maxAttempts} attempts rejected. Using best candidate (score: ${bestScore.toFixed(2)}). Issues: ${lastQuality?.summary || "unknown"}`);
  reportProgress(hooks, 44, "critic", `Critic accepted ${traceLabel} (best of ${maxAttempts} attempts)`);

  return bestCandidate || buildScreenFromWriterResult({
    baseScreen: screen,
...
// Score a candidate for comparison across attempts (higher = better)
function computeCandidateScore(candidate, quality) {
  let score = 0;
  const textLen = (candidate.bodyLong || "").length;
  
  // Length contributes 0-40 points
  score += Math.min(40, textLen / 25);
  
  // No critical problems = +30
  const critical = (quality.problems || []).filter(p => p.severity === "critical").length;
  score += Math.max(0, 30 - critical * 15);
  
  // No major problems = +20
  const major = (quality.problems || []).filter(p => p.severity === "major").length;
  score += Math.max(0, 20 - major * 10);
  
  // Has key takeaways = +10
  if (Array.isArray(candidate.keyTakeaways) && candidate.keyTakeaways.length >= 2) {
    score += 10;
  }
...
async function buildFinalTestFromScreens(course, input, hooks, ragContext = null) {
  ...
  reportProgress(hooks, 84, "test-builder", input.isQuizOnly ? "Building Quiz directly from RAG Context" : "Building final test from approved screens");
  ...
  if (screens.length === 0 && !input.isQuizOnly) {
    return {
  ...
  if (input.isQuizOnly && ragContext?.chunks?.length > 0) {
    combinedCourseText = ragContext.chunks.map(c => c.text).join("\n\n").slice(0, 24000);
  } else {
    combinedCourseText = screens.map(s => `=== ${s.title} ===\n${s.bodyLong}\n${(s.keyTakeaways||[]).map(t => "- " + t).join("\n")}`).join("\n\n");
  }

  const aiQuestions = await createFinalTestFromLocalLlm(input, combinedCourseText, { trace: { phase: "final-test" } });

  // QA Agent: validate generated questions
  const { validateTestQuestions } = await import("./qa-agent.js");

  let questions = [];

  if (aiQuestions && Array.isArray(aiQuestions) && aiQuestions.length >= desiredCount) {
    // Pre-validate raw questions before formatting
    const rawFormatted = aiQuestions.slice(0, desiredCount + 2).map((q, index) => {
    ...
    const validated = await validateTestQuestions(input, rawFormatted, combinedCourseText);
    questions = validated.slice(0, desiredCount);
  } else if (!input.isQuizOnly) {
...
async function runV4Pipeline(course, input, ragContext, plan, hooks) {
    ...
    for (let sectionIndex = 0; sectionIndex < (moduleItem.sections || []).length; sectionIndex += 1) {
      ...
      for (let scoIndex = 0; scoIndex < (section.scos || []).length; scoIndex += 1) {
        ...
        for (let screenIndex = 0; screenIndex < (sco.screens || []).length; screenIndex += 1) {
          ...
          // Editor Agent: refine text if quality issues detected
          const refinedScreen = await refineScreenText(input, updatedScreen);
  ...
  course.finalTest = await buildFinalTestFromScreens(course, input, hooks, ragContext);
