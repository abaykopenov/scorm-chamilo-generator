...
import { buildTemplateDraft } from "./generation/template-draft.js";
import { evaluateLinePlanQuality } from "./course-utils.js";
...
export async function generateCourseDraft(payload, hooks = {}) {
  ...
  if (input.isQuizOnly) {
    reportProgress(hooks, 10, "planner", "Skipping course outline generation for Quiz-only mode");
    let course = {
      title: input.titleHint || "Knowledge Quiz",
      description: "Auto-generated knowledge test",
      modules: [],
      finalTest: input.finalTest
    };
    course.finalTest = await buildFinalTestFromScreens(course, input, hooks, initialRagContext);
    return attachRagMetadata(course, input, initialRagContext);
  }
  ...
  const preferSegmented = batchOnly || providerName === "ollama" || providerName === "openai-compatible" || shouldPreferSegmentedGeneration(input);
