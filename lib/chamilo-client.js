...
function buildExerciseQuestionRequestBody(questionPayload, protectToken) {
    const bodyParts = [];
...
async function addQuestionToExercise({
    ...
    const body = buildExerciseQuestionRequestBody(normalizedQuestion, protectToken);
    const response = await requestWithCookieJar(cookieJar, qUrl, {
        method: "POST",
        headers: {
            "Content-Type": "application/x-www-form-urlencoded"
        },
...
export async function addExerciseToLearningPath({profile, lpId, exerciseId, exerciseTitle, cookieJar: existingJar}) {
    ...
    const body = new URLSearchParams();
    body.set("title", trimText(exerciseTitle || "Final test") || "Final test");
    body.set("parent", "0");
