...
export async function createChamiloExercise({ profile, exercise }) {
  ...
  for (let qIdx = 0; qIdx < questions.length; qIdx++) {
    ...
    const numOptions = options.length || 4;

    try {
      // ── Step 1: GET the combined question+answers form ──
      const qFormUrl = `${base}/main/exercise/admin.php?cidReq=${cidReq}&exerciseId=${exerciseId}&newQuestion=yes&answerType=1`;
      console.log(`[Q${qIdx + 1}] GET form: ${qFormUrl}`);
      const { response: qFormResp, url: qUrl } = await followRedirects(qFormUrl, cookieJar);
      const qFormHtml = await qFormResp.text();

      // Extract ALL hidden fields — robust parser
      const hiddenFields = {};
      const inputRe = /<input[^>]*>/gi;
      let im;
      while ((im = inputRe.exec(qFormHtml))) {
        const tag = im[0];
      ...
      // POST to the SAME URL as GET (must include exerciseId)
      const postUrl = qFormUrl;

      // ── Step 2: POST everything in ONE request ──
      // Use correctOptionIndex for LLM-generated questions
      let correctNum = 1;
      if (typeof q.correctOptionIndex === "number") {
        correctNum = q.correctOptionIndex + 1;
      } else {
        const correctIdx = options.findIndex((opt) => opt.id === q.correctOptionId);
        correctNum = correctIdx >= 0 ? correctIdx + 1 : 1;
      }

      // Build body manually to avoid URLSearchParams encoding brackets as %5B/%5D
      const parts = [];
      function add(k, v) { parts.push(encodeURIComponent(k) + "=" + encodeURIComponent(v)); }

      // Hidden fields
      for (const [k, v] of Object.entries(hiddenFields)) add(k, v);
      // Question
      add("questionName", q.prompt || `Вопрос ${qIdx + 1}`);
      add("questionDescription", "");
      add("questionLevel", "1");
      add("questionCategory", "0");
      // Answers
      add("nb_answers", String(numOptions));
      add("correct", String(correctNum));
      for (let i = 0; i < numOptions; i++) {
        const num = i + 1;
        const text = options[i]?.text || options[i] || `Вариант ${num}`;
        const ansText = typeof text === "string" ? text : String(text);
        // answer[1] must NOT be URL-encoded brackets - use raw key
        parts.push(`answer%5B${num}%5D=` + encodeURIComponent(ansText));
        parts.push(`comment%5B${num}%5D=`);
        parts.push(`weighting%5B${num}%5D=` + (num === correctNum ? "10" : "0"));
        parts.push(`counter%5B${num}%5D=${num}`);
      }
      // Submit button
      // Submit button
      add("submitQuestion", "");
      ...
      console.log(`[Q${qIdx + 1}] POST to: ${postUrl} (body ${bodyStr.length} chars)`);
      ...
      console.log(`[Q${qIdx + 1}] Response: status=${resp.status}, location=${resp.headers.get("location") || "none"}`);

      // Follow any redirect
      if (resp.status >= 300 && resp.status < 400) {
        const loc = resp.headers.get("location");
        if (loc) await followRedirects(new URL(loc, postUrl).toString(), cookieJar);
      }

      results.push({ question: qIdx + 1, ok: true, status: resp.status });
    } catch (qErr) {
      console.error(`[Q${qIdx + 1}] Error:`, qErr.message);
