...
export async function saveCourse(course) {
  ...
  const generation = course.generation
    ? (({ apiKey, ...rest }) => rest)(course.generation)
    : course.generation;
  const payload = {
    ...course,
    generation,
...
export async function deleteCourse(courseId) {
  ...
  try {
    ...
    // Also remove export files if they exist
    const zipPath = path.join(EXPORTS_DIR, `${courseId}.zip`);
    const metaPath = path.join(EXPORTS_DIR, `${courseId}.json`);
