...
function looksCorruptedNarrative(value) {
...
function buildInlineScreenIllustration(title, courseTitle) {
...
function ensureImageBlock(blocks, title, courseTitle) {
    ...
    for (const section of sections) {
      ...
      for (const sco of scos) {
        ...
        for (let screenIndex = 0; screenIndex < screens.length; screenIndex += 1) {
          ...
          const blocks = (Array.isArray(screen?.blocks) ? [...screen.blocks] : []).filter((block) => block?.type !== "note" && block?.type !== "image");
