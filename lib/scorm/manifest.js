...
${sectionItems}
  ...
  if (course.finalTest?.enabled) {
    const resourceId = `RES_${course.finalTest.id}`;
    resourceEntries.push(
      `<resource identifier="${resourceId}" type="webcontent" adlcp:scormtype="sco" href="sco/${course.finalTest.id}.html">
  <file href="sco/${course.finalTest.id}.html" />
</resource>`
    );
    itemEntries.push(
      `    <item identifier="ITEM_${course.finalTest.id}" identifierref="${resourceId}">
      <title>${escapeXml(course.finalTest.title)}</title>
      <adlcp:masteryscore>${course.finalTest.passingScore}</adlcp:masteryscore>
    </item>`
    );
  }
