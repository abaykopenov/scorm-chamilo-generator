...
export function createRuntimeAssets(course) {
  ...
  function renderBlocks(blocks) {
    return blocks.map(function (block) {
      ...
      if (block.type === "table") {
        var html = '<div class="table-wrapper"><table style="width:100%;border-collapse:collapse;margin:1rem 0;">';
        if (block.columns && block.columns.length) {
          html += '<thead><tr>' + block.columns.map(function(c) { return '<th style="border:1px solid #ddd;padding:12px;background:#f8fafc;font-weight:600;text-align:left;">' + escapeHtml(String(c)) + '</th>'; }).join('') + '</tr></thead>';
        }
        if (block.rows && block.rows.length) {
          html += '<tbody>' + block.rows.map(function(row) {
            return '<tr>' + (row || []).map(function(cell) { return '<td style="border:1px solid #ddd;padding:12px;">' + escapeHtml(String(cell)) + '</td>'; }).join('') + '</tr>';
          }).join('') + '</tbody>';
        }
        html += '</table></div>';
        return html;
  ...
  try {
    ...
    var app = document.getElementById("app");
    if (app) {
      app.innerHTML = '<div class="card" style="color:red;padding:20px;"><h2>Ошибка SCORM Runtime</h2><pre>' + escapeHtml(String(err && err.stack || err)) + '</pre><p>Screens: ' + (runtime.screens ? runtime.screens.length : 'undefined') + '</p></div>';
    }
    console.error("SCORM Runtime Error:", err);
  }
})();`
    },
    {
