import { repairKazakhText } from "./kazakh-text-repair.js";
...
function buildSourceContext(input, limits = {}) {
  ...
  if (!Array.isArray(input?.ragContext?.chunks)) return [];

  return input.ragContext.chunks
    .slice(0, maxItems + 5) // take a few extra in case some get filtered
    .map((chunk, index) => {
      const cleanedText = sanitizeChunkText(`${chunk.text || ""}`.slice(0, maxChars));
...
/**
 * Sanitize a RAG chunk text before passing it to LLM prompt.
 * Removes garbled/merged words, pipe-table artifacts, metadata fragments,
 * university headers, math formula noise, and other Docling artifacts.
 */
function sanitizeChunkText(text) {
  ...
  // Split into lines and filter garbage lines
  const lines = cleaned.split(/\n/);
  const goodLines = lines.filter(line => {
    const trimmed = line.trim();
    if (!trimmed) return false;
    // Skip ANY line with pipe characters (table artifacts)
    if (trimmed.includes("|")) return false;
    // Skip lines with merged Cyrillic words (16+ consecutive chars — lowered from 18)
    if (/[\u0400-\u04FF]{16,}/.test(trimmed)) return false;
    // Skip university header
    if (/ӘЛ.?ФАРАБ/iu.test(trimmed) || /ҚАЗАҚ\s*ҰЛТТЫҚ\s*УНИВЕРСИТЕТ/iu.test(trimmed)) return false;
    // Skip metadata lines
    if (/Научныйредактор|Авторывыражают|поблагодарить|ISBN[\s:\-]*[\dXx\-]{10,}/i.test(trimmed)) return false;
    // Skip mathematical formula noise
    if (/(?:chchch|ch\s*ch|ijo\b|hэлемент|kchchch|\.{3,}\s*,\s*\.{3,})/i.test(trimmed)) return false;
    // Skip figure/table references: "2.3-сурет", "Сурет 1.4", "Кесте 2.1", "рисунок", "таблица"
    if (/(?:^|\s)(?:\d+[\.\-]\d*\s*[-–]?\s*)?(?:сурет|кесте|figure|table|fig\.|рис\.|таблица|рисунок)\s*[\d.]*\s*$/i.test(trimmed) && trimmed.length < 80) return false;
    if (/^(?:сурет|кесте|figure|table|рис\.|таблица|рисунок)\s+\d/i.test(trimmed) && trimmed.length < 80) return false;
    // Skip formula-variable lines: "j A", "X F k", "k1 , k2", "(2.8)"
    const formulaChars = (trimmed.match(/[\d^/(){}=+*,.\[\]<>_]/g) || []).length;
    const alphaChars = (trimmed.match(/\p{L}/gu) || []).length;
    if (alphaChars > 0 && formulaChars / (alphaChars + formulaChars) > 0.4 && trimmed.length < 60) return false;
    // Skip lines that are mostly tiny fragments (broken OCR)
    const words = trimmed.split(/\s+/);
    if (words.length > 5) {
      const tinyFragments = words.filter(w => /^[\u0400-\u04FF]{1,2}$/.test(w));
      if (tinyFragments.length / words.length > 0.4) return false;
    }
    // Skip very short lines (page numbers, artifacts)
    if (trimmed.length < 10) return false;
    return true;
  });

  cleaned = goodLines.join(" ").replace(/\s+/g, " ").trim();

  // Remove inline math formula artifacts: "ChG", "  R ", "  С Ch", "  0  "
  cleaned = cleaned.replace(/\s+[A-Z][a-z]?[A-Z]\s+/g, " ");
  // Remove isolated formula fragments: "hk chchch ,..., , 2 1"
  cleaned = cleaned.replace(/\b[hkn]\s*(?:chchch|chch|ch)\b[^.]*?(?:\d\s*)+/gi, "");
  // Remove formula-like number sequences: ",..., , 2 1"
  cleaned = cleaned.replace(/,\s*\.\.\.\s*,\s*,?\s*\d+\s*\d*/g, "");

  // Apply comprehensive Kazakh text repair (fragments, merges, postpositions)
  cleaned = repairKazakhText(cleaned);

  // Remove any remaining very short fragments at start (from truncated chunks)
  cleaned = cleaned.replace(/^[а-яёәғқңөұүіһ]{1,4}\s+/i, "");

  cleaned = cleaned.replace(/\s{2,}/g, " ").trim();

  // ---- Quality gate: reject chunks that are too short after cleaning ----
  const remainingLetters = (cleaned.match(/\p{L}/gu) || []).length;
  if (remainingLetters < 50) return "";
...
export function createOutlinePrompt(input) {
  ...
  const system = [
    ...
    // ===== QUALITY RULES =====
    "QUALITY RULE — NO FILLER TEXT: Do not write generic introductory phrases like 'it is important to note', 'in the framework of', 'it should be understood that', 'in this section we will discuss'. Every sentence must carry NEW information. If a sentence can be removed without losing information, do not write it. NEVER start a screen with 'В этом разделе мы рассмотрим' or similar meta-commentary.",
    "QUALITY RULE — NO REPETITION ACROSS SCREENS: Each screen MUST cover a DIFFERENT topic. NEVER repeat definitions, lists, or explanations from previous screens. If you already defined 'охрана труда' on one screen, do NOT define it again. Each screen must introduce entirely NEW concepts.",
    "QUALITY RULE — UNIQUE MODULE NAMES: Every module MUST have a unique, specific name. NEVER give two modules the same or similar name (e.g. 'Основы охраны труда' and 'Введение в охрану труда' are TOO SIMILAR). Each module name must clearly indicate its unique subtopic.",
    ...
    Array.isArray(input.moduleMemory) && input.moduleMemory.length > 0
      ...
      : "",
    "CRITICAL QUALITY CHECK: Ensure perfect spelling and flowing, natural grammar strictly in the requested language.",
    ...
    input.language === "en"
      ...
      : input.language === "kk"
        ? "LANGUAGE: Generate ALL content (titles, descriptions, text blocks, list items, test questions, and explanations) in KAZAKH (Қазақ тілі). Do NOT mix languages. Use proper Kazakh grammar and vocabulary."
...
export function createFinalTestPrompt(input, combinedCourseText) {
  ...
  const lang = input?.language === "en" ? "English" : input?.language === "kk" ? "Kazakh (Қазақ тілі)" : "Russian";
  ...
  return {
    system: [
      ...
      "CRITICAL RULE 8 — DISTRIBUTE CORRECT ANSWERS: The correctOptionIndex MUST vary across questions. Do NOT set correctOptionIndex to 0 for every question. Distribute correct answers roughly equally across positions 0, 1, 2, 3. For example, if there are 8 questions, approximately 2 should have correctOptionIndex=0, 2 with =1, 2 with =2, 2 with =3.",
