...
export function createOutlinePrompt(input) {
  ...
  const system = [
    ...
    structureOnly
      ...
      : (writerPhase
        ? "For each screen return: 1) one long text block (minimum about 900 characters), 2) one list block (3-5 concrete takeaways), 3) one note block with actual short quote snippets as source references."
        : "Do not add any extra fields outside schema."),
    writerPhase
      ? "Do not invent facts beyond sourceContext. Every claim must be traceable to provided facts. NEVER mention source file names or hint indices in the educational text directly. NEVER write generic 'Source X, Y, Z' in notes."
    ...
    "CRITICAL: All generated 'title' fields (for course, modules, sections, scos, screens) MUST be descriptive and reflect the actual content (e.g. 'Introduction to Digital Twins'). NEVER use generic placeholder titles like 'Module 1', 'Section 2', 'SCO 3', or 'Screen 4'.",
