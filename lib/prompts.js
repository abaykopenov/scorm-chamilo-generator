...
export function createOutlinePrompt(input) {
  ...
  const system = [
    ...
    structureOnly
      ? "For each screen return one short text block. Avoid long narratives and avoid list blocks unless required."
      : (writerPhase
        ? "For each screen return: 1) one substantive text block, 2) one list block with 3-5 concrete takeaways."
    ...
    "QUALITY RULE — NO REPETITION: Never rephrase the same idea in different words within the same screen. Each paragraph must introduce a new concept, fact, or procedure. If you catch yourself restating what was already said, stop and move to the next point.",
    ...
    "QUALITY RULE — DIRECT QUOTES FROM EVIDENCE: When sourceContext provides factual content, directly quote the key phrases and terminology from it. Do NOT paraphrase or generalize. Use the exact wording from the source documents. If the source says 'процедура и формы хранения результатов обучения', write exactly that, do not rephrase it as 'results should be stored appropriately'.",
...
export function createLinePlanPrompt(input) {
  ...
  return {
    system: [
      ...
      "QUALITY RULE — DIRECT QUOTES: Use exact wording and terminology from sourceContext. Do not paraphrase key terms.",
