...
function normalizeTitle(value, fallback) {
  // Очистка заголовка от переносов строк и лишних пробелов
  const text = toText(value, fallback) || fallback;
  return text
    .replace(/\r\n/g, ' ')
    .replace(/\n/g, ' ')
    .replace(/\r/g, ' ')
...
export function normalizeGenerateInput(payload) {
  ...
  return {
    titleHint: normalizeTitle(payload?.titleHint, defaults.titleHint),
