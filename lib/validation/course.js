...
function normalizeBlocks(blocks, screenTitle) {
  ...
  return blocks
    .map((block) => {
      ...
      const type = ["text", "note", "list", "table", "image"].includes(block.type) ? block.type : "text";
      if (type === "list") {
      ...
      if (type === "table") {
        return {
          type,
          columns: toTextArray(block.columns, []),
          rows: Array.isArray(block.rows) ? block.rows.map(row => toTextArray(row, [])) : []
        };
      }
      if (type === "image") {
        return {
    ...
    ? questions.map((question, index) => {
        const rawOptions = Array.isArray(question?.options) && question.options.length > 0
          ? question.options.map((option, optionIndex) => ({
              ...
              text: toText(option?.text ?? option, `Option ${optionIndex + 1}`)
            }))
          : Array.from({ length: 4 }, (_, i) => ({ id: createId("option"), text: `Option ${i + 1}` }));

        // Determine which option is correct
        let correctIdx = Number(question?.correctOptionIndex);
        if (!Number.isFinite(correctIdx) || correctIdx < 0 || correctIdx >= rawOptions.length) {
          correctIdx = 0;
        }
        const correctId = question?.correctOptionId || rawOptions[correctIdx]?.id;

        // Shuffle options so correct answer isn't always at position 0
