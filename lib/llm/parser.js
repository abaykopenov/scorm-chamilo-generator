...
export function validateOutlineJson(parsed, input, validate = {}) {
  ...
  if (Number.isFinite(expectedModules) && expectedModules > 0) {
    const minAcceptable = Math.max(1, Math.floor(expectedModules * 0.5));
    if (modules.length < minAcceptable) {
      return { ok: false, reason: "expected-modules-" + expectedModules + "-got-" + modules.length };
    }
    if (modules.length !== expectedModules) {
      console.warn("[outline-validation] Module count mismatch: expected " + expectedModules + ", got " + modules.length + " (accepted, will pad with fallbacks)");
    }
  }
  ...
  if (Number.isFinite(expectedSections) && expectedSections > 0) {
    if (firstSections.length === 0) {
      return { ok: false, reason: "expected-sections-" + expectedSections + "-got-0" };
    }
    if (firstSections.length !== expectedSections) {
      console.warn("[outline-validation] Section count mismatch: expected " + expectedSections + ", got " + firstSections.length + " (accepted)");
    }
  }

  const firstScos = Array.isArray(firstSections?.[0]?.scos) ? firstSections[0].scos : [];
  const expectedScos = Number(validate?.expectedScos);
  if (Number.isFinite(expectedScos) && expectedScos > 0) {
    if (firstScos.length === 0) {
      return { ok: false, reason: "expected-scos-" + expectedScos + "-got-0" };
    }
    if (firstScos.length !== expectedScos) {
      console.warn("[outline-validation] SCO count mismatch: expected " + expectedScos + ", got " + firstScos.length + " (accepted)");
    }
  }
  ...
  if (Number.isFinite(expectedScreens) && expectedScreens > 0) {
    if (firstScreens.length === 0) {
      return { ok: false, reason: "expected-screens-" + expectedScreens + "-got-0" };
    }
    if (firstScreens.length !== expectedScreens) {
      console.warn("[outline-validation] Screen count mismatch: expected " + expectedScreens + ", got " + firstScreens.length + " (accepted)");
    }
  }
