  ...
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    ...
    try {
      ...
      if (response.status === 429 && attempt < maxRetries) {
        ...
        // If Retry-After > 120s, daily/hourly quota exhausted — bail out immediately
        if (Number.isFinite(retryAfterSec) && retryAfterSec > 120) {
          llmLog("http.rate-limit-exhausted", {
            label, url, attempt, retryAfterSec,
            ...describeTrace(meta?.trace),
            stage: meta?.stage
          });
          return response;
        ...
        const waitMs = Number.isFinite(retryAfterSec) && retryAfterSec > 0
          ...
          : Math.min(15_000 * (attempt + 1), 60_000);
        llmLog("http.rate-limit-retry", {
          label, url, attempt, waitMs,
